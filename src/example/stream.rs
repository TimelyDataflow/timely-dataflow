...
pub struct Stream<T: Timestamp, S: PathSummary<T>, D:Copy+'static>
{
    ...
    pub port:       Rc<RefCell<Vec<Box<TargetPort<T, D>>>>>,    // used to register interest in the output.
...
{
    pub fn copy_with<D2: Copy+'static>(&self, name: Source, port: Rc<RefCell<Vec<Box<TargetPort<T, D2>>>>>) -> Stream<T, S, D2>
    {
        Stream
        {
            name: name,
