...
impl<'a, G: Graph+'a, D:Data> Stream<'a, G, D> {
    pub fn new(source: Source, output: OutputPort<G::Timestamp, D>, graph: &'a RefCell<G>) -> Self {
        Stream {
            name: source,
            ports: output,
            graph: graph,
    ...
    pub fn clone_with<D2: Data>(&self, source: Source, targets: OutputPort<G::Timestamp, D2>) -> Stream<'a, G, D2> {
