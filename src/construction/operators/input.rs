...
pub trait Extension<A: Allocate, T: Timestamp+Ord> {
    fn new_input<D:Data>(&self) -> (InputHelper<T, D>, Stream<SubgraphBuilder<GraphRoot<A>, T>, D>);
}

impl<A: Allocate, T: Timestamp+Ord> Extension<A, T> for SubgraphBuilder<GraphRoot<A>, T> {
    fn new_input<D:Data>(&self) -> (InputHelper<T, D>, Stream<SubgraphBuilder<GraphRoot<A>, T>, D>) {
