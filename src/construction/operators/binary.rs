...
pub trait Extension<G: GraphBuilder, D1: Data> {
    fn binary_stream<D2: Data,
              ...
              L: FnMut(&mut PullableCounter<G::Timestamp, D1>,
                       &mut PullableCounter<G::Timestamp, D2>,
                       &mut ObserverBuffer<G::Timestamp, D3, ObserverCounter<G::Timestamp, D3, Tee<G::Timestamp, D3>>>)+'static,
    ...
    fn binary_notify<D2: Data,
              ...
              L: FnMut(&mut PullableCounter<G::Timestamp, D1>,
                       &mut PullableCounter<G::Timestamp, D2>,
                       &mut ObserverBuffer<G::Timestamp, D3, ObserverCounter<G::Timestamp, D3, Tee<G::Timestamp, D3>>>,
...
impl<G: GraphBuilder, D1: Data> Extension<G, D1> for Stream<G, D1> {
    fn binary_stream<
        ...
        let operator = Operator::new(PullableCounter::new(receiver1), PullableCounter::new(receiver2), targets, name.to_owned(), None, move |i1, i2, o, _| logic(i1, i2, o));
    ...
    fn binary_notify<
        ...
        let (sender2, receiver2) = pact2.connect(&mut builder);;
        let (targets, registrar) = Tee::<G::Timestamp,D3>::new();
        let operator = Operator::new(PullableCounter::new(receiver1), PullableCounter::new(receiver2), targets, name.to_owned(), Some((notify, builder.peers())), logic);
        let index = builder.add_operator(operator);
        self.connect_to(ChildInput(index, 0), sender1);
        other.connect_to(ChildInput(index, 1), sender2);
        // self.builder.connect(other.name, ChildInput(index, 1));
...
struct Handle<T: Timestamp, D1: Data, D2: Data, D3: Data> {
    pub input1:         PullableCounter<T, D1>,
    pub input2:         PullableCounter<T, D2>,
    pub output:         ObserverBuffer<T, D3, ObserverCounter<T, D3, Tee<T, D3>>>,
...
struct Operator<T: Timestamp,
                       ...
                       L: FnMut(&mut PullableCounter<T, D1>,
                                &mut PullableCounter<T, D2>,
                                &mut ObserverBuffer<T, D3, ObserverCounter<T, D3, Tee<T, D3>>>,
                                &mut Notificator<T>)> {
    name:           String,
...
Operator<T, D1, D2, D3, L> {
    pub fn new(receiver1: PullableCounter<T, D1>,
               receiver2: PullableCounter<T, D2>,
               targets: Tee<T, D3>,
               name: String,
