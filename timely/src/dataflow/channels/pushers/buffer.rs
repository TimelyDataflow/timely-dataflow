...
/// Buffers data sent at the same time, specialized to vectors.
...
impl<'a, T, C: Container, P: Push<BundleCore<T, C>, MessageAllocation<C::Allocation>>+'a> Session<'a, T, C, P>  where T: Eq+Clone+'a, C: 'a {
    ...
    #[inline]
    pub fn give_container(&mut self, message: C, allocation: &mut Option<C::Allocation>) {
        if !message.is_empty() {
            self.buffer.give_container(message, allocation);
        }
    }
}
