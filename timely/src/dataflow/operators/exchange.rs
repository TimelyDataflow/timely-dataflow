...
/// Exchange records between workers.
pub trait Exchange<T: 'static, D: ExchangeData> {
    /// Exchange records between workers.
    ///
    ...
    /// timely::example(|scope| {
    ///     (0..10).to_stream(scope)
    ///            .exchange_pact(LazyExchange::new(|x| *x))
    ///            .inspect(|x| println!("seen: {:?}", x));
    /// });
    /// ```
    fn exchange_pact<P: ParallelizationContract<T, D>>(&self, pact: P) -> Self;
}

impl<G: Scope, D: ExchangeData> Exchange<G::Timestamp, D> for Stream<G, D> {
    fn exchange(&self, route: impl Fn(&D)->u64+'static) -> Stream<G, D> {
        self.exchange_pact(ExchangePact::new(route))
    }

    fn exchange_pact<P: ParallelizationContract<G::Timestamp, D>>(&self, pact: P) -> Stream<G, D> {
