...
pub trait Map<S: Scope, D: Data> {
    ...
    /// });
    /// ```
    fn map<D2: Data, L: FnMut(D)->D2+'static>(self, logic: L) -> OwnedStream<S, Vec<D2>>;
    ...
    /// });
    /// ```
    fn map_in_place<L: FnMut(&mut D)+'static>(self, logic: L) -> OwnedStream<S, Vec<D>>;
    ...
    /// });
    /// ```
    fn flat_map<I: IntoIterator, L: FnMut(D)->I+'static>(self, logic: L) -> OwnedStream<S, Vec<I::Item>> where I::Item: Data;
}

impl<G: Scope, D: Data, S: StreamLike<G, Vec<D>>> Map<G, D> for S {
    fn map<D2: Data, L: FnMut(D)->D2+'static>(self, mut logic: L) -> OwnedStream<G, Vec<D2>> {
    ...
    fn flat_map<I: IntoIterator, L: FnMut(D)->I+'static>(self, mut logic: L) -> OwnedStream<G, Vec<I::Item>> where I::Item: Data {
        let mut vector = Vec::new();
