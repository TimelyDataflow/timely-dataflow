...
pub trait Map<S: Scope, D: 'static> : Sized {
    ...
    /// });
    /// ```
    fn map<D2: 'static, L: FnMut(D)->D2+'static>(self, mut logic: L) -> Stream<S, D2> {
    ...
    /// });
    /// ```
    fn flat_map<I: IntoIterator, L: FnMut(D)->I+'static>(self, logic: L) -> Stream<S, I::Item> where I::Item: 'static;
}

impl<S: Scope, D: 'static> Map<S, D> for Stream<S, D> {
