...
pub trait Input : Scope {
    ...
    /// });
    /// ```
    fn new_input_core<D: Container + Data>(&mut self) -> (HandleCore<<Self as ScopeParent>::Timestamp, D>, StreamCore<Self, D>);
    ...
    /// });
    /// ```
    fn input_from_core<D: Container + Data>(&mut self, handle: &mut HandleCore<<Self as ScopeParent>::Timestamp, D>) -> StreamCore<Self, D>;
}
...
impl<G: Scope> Input for G where <G as ScopeParent>::Timestamp: TotalOrder {
    ...
    fn new_input_core<D: Container + Data>(&mut self) -> (HandleCore<<G as ScopeParent>::Timestamp, D>, StreamCore<G, D>) {
        let mut handle = HandleCore::new();
        let stream = self.input_from_core(&mut handle);
        (handle, stream)
    }

    fn input_from_core<D: Container + Data + std::clone::Clone>(&mut self, handle: &mut HandleCore<<G as ScopeParent>::Timestamp, D>) -> StreamCore<G, D> {
