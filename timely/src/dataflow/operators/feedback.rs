...
pub trait Feedback<G: Scope> {
    ...
    /// });
    /// ```
    fn feedback<D: Data>(&mut self, summary: <G::Timestamp as Timestamp>::Summary) -> (Handle<G, D>, OwnedStream<G, Vec<D>>);
    ...
    /// });
    /// ```
    fn feedback_core<D: Container>(&mut self, summary: <G::Timestamp as Timestamp>::Summary) -> (HandleCore<G, D>, OwnedStream<G, D>);
}
...
pub trait LoopVariable<'a, G: Scope, T: Timestamp> {
    ...
    /// });
    /// ```
    fn loop_variable<D: Container>(&mut self, summary: T::Summary) -> (HandleCore<Iterative<'a, G, T>, D>, OwnedStream<Iterative<'a, G, T>, D>);
}
...
pub struct HandleCore<G: Scope, D: Container> {
    ...
    output: OutputWrapper<G::Timestamp, D, PushOwned<G::Timestamp, D>>,
}
