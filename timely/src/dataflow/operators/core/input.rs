...
pub trait Input : Scope {
    ...
    /// });
    /// ```
    fn new_input<C: WithProgress + Data>(&mut self) -> (Handle<<Self as ScopeParent>::Timestamp, CapacityContainerBuilder<C>>, StreamCore<Self, C>);
...
impl<G: Scope> Input for G where <G as ScopeParent>::Timestamp: TotalOrder {
    fn new_input<C: WithProgress + Data>(&mut self) -> (Handle<<G as ScopeParent>::Timestamp, CapacityContainerBuilder<C>>, StreamCore<G, C>) {
...
impl<T: Timestamp, C: WithProgress + Data> Handle<T, CapacityContainerBuilder<C>> {
