...
pub trait Feedback<G: Scope> {
    /// Creates a [StreamCore] and a [Handle] to later bind the source of that `Stream`.
    ///
    ...
    /// use timely::dataflow::operators::core::{Feedback, ConnectLoop};
    ...
    /// });
    /// ```
    fn feedback<D: Container>(&mut self, summary: <G::Timestamp as Timestamp>::Summary) -> (Handle<G, D>, StreamCore<G, D>);
}
...
pub trait LoopVariable<'a, G: Scope, T: Timestamp> {
    ...
    /// });
    /// ```
    fn loop_variable<D: Container>(&mut self, summary: T::Summary) -> (Handle<Iterative<'a, G, T>, D>, StreamCore<Iterative<'a, G, T>, D>);
}
