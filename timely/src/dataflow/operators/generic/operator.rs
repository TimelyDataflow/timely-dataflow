...
pub trait Operator<G: Scope, C1: Container> {
    ...
    where
        ...
        L: FnMut(&mut FrontieredInputHandleCore<G::Timestamp, C1, P::Puller>,
                 &mut OutputHandleCore<G::Timestamp, CB, PushOwned<G::Timestamp, CB::Container>>)+'static,
...
where
    CB: ContainerBuilder,
    B: FnOnce(Capability<G::Timestamp>, OperatorInfo) -> L,
    L: FnMut(&mut OutputHandleCore<G::Timestamp, CB, PushOwned<G::Timestamp, CB::Container>>)+'static {
