...
pub struct InputHandleCore<T: Timestamp, D, A, P: Pull<BundleCore<T, D>, MessageAllocation<A>>> {
    pull_counter: PullCounter<T, D, A, P>,
    internal: Rc<RefCell<Vec<Rc<RefCell<ChangeBatch<T>>>>>>,
    logging: Option<Logger>,
}

/// Handle to an operator's input stream, specialized to vectors.
pub type InputHandle<T, D, P> = InputHandleCore<T, Vec<D>, Vec<D>, P>;

/// Handle to an operator's input stream and frontier.
pub struct FrontieredInputHandleCore<'a, T: Timestamp, D: 'a, A: 'a, P: Pull<BundleCore<T, D>, MessageAllocation<A>>+'a> {
    /// The underlying input handle.
    pub handle: &'a mut InputHandleCore<T, D, A, P>,
...
pub type FrontieredInputHandle<'a, T, D, P> = FrontieredInputHandleCore<'a, T, Vec<D>, Vec<D>, P>;

impl<'a, T: Timestamp, D: Container, P: Pull<BundleCore<T, D>, MessageAllocation<D::Allocation>>> InputHandleCore<T, D, D::Allocation, P> {
...
impl<'a, T: Timestamp, D: Container, P: Pull<BundleCore<T, D>, MessageAllocation<D::Allocation>>+'a> FrontieredInputHandleCore<'a, T, D, D::Allocation, P> {
    /// Allocate a new frontiered input handle.
    pub fn new(handle: &'a mut InputHandleCore<T, D, D::Allocation, P>, frontier: &'a MutableAntichain<T>) -> Self {
...
pub fn _access_pull_counter<T: Timestamp, D, A, P: Pull<BundleCore<T, D>, MessageAllocation<A>>>(input: &mut InputHandleCore<T, D, A, P>) -> &mut PullCounter<T, D, A, P> {
...
pub fn new_input_handle<T: Timestamp, D, A, P: Pull<BundleCore<T, D>, MessageAllocation<A>>>(pull_counter: PullCounter<T, D, A, P>, internal: Rc<RefCell<Vec<Rc<RefCell<ChangeBatch<T>>>>>>, logging: Option<Logger>) -> InputHandleCore<T, D, A, P> {
    InputHandleCore {
        pull_counter,
        internal,
        logging,
...
#[derive(Debug)]
pub struct OutputWrapper<T: Timestamp, D: Container, P: Push<BundleCore<T, D>, MessageAllocation<D::Allocation>>> {
    push_buffer: BufferCore<T, D, PushCounter<T, D, D::Allocation, P>>,
    internal_buffer: Rc<RefCell<ChangeBatch<T>>>,
}

impl<T: Timestamp, D: Container, P: Push<BundleCore<T, D>, MessageAllocation<D::Allocation>>> OutputWrapper<T, D, P> {
    /// Creates a new output wrapper from a push buffer.
    pub fn new(push_buffer: BufferCore<T, D, PushCounter<T, D, D::Allocation, P>>, internal_buffer: Rc<RefCell<ChangeBatch<T>>>) -> Self {
        OutputWrapper {
            push_buffer,
            internal_buffer,
        }
...
pub struct OutputHandleCore<'a, T: Timestamp, C: Container+'a, P: Push<BundleCore<T, C>, MessageAllocation<C::Allocation>>+'a> {
    push_buffer: &'a mut BufferCore<T, C, PushCounter<T, C, C::Allocation, P>>,
...
impl<'a, T: Timestamp, C: Container, P: Push<BundleCore<T, C>, MessageAllocation<C::Allocation>>> OutputHandleCore<'a, T, C, P> {
    ...
    /// });
    /// ```
    pub fn session<'b, CT: CapabilityTrait<T>>(&'b mut self, cap: &'b CT) -> Session<'b, T, C, PushCounter<T, C, C::Allocation, P>> where 'a: 'b {
...
impl<'a, T: Timestamp, C: Container, P: Push<BundleCore<T, C>, MessageAllocation<C::Allocation>>> Drop for OutputHandleCore<'a, T, C, P> {
    fn drop(&mut self) {
