...
impl<A: Allocate> Worker<A> {
    ...
    where
        T: Refines<()>,
        F: FnOnce(&mut Child<Self, T, SubgraphBuilder<(), T>>)->R,
    {
    ...
    where
        T: Refines<()>,
        F: FnOnce(&mut V, &mut Child<Self, T, SubgraphBuilder<(), T>>)->R,
        V: Any+'static,
    {
        self.dataflow_subgraph::<_, _, _, _, SubgraphBuilder<_, _>>(name, logging, resources, func)
    }
    ...
    /// });
    /// ```
    pub fn dataflow_subgraph<T, R, F, V, SG>(&mut self, name: &str, mut logging: Option<TimelyLogger>, mut resources: V, func: F) -> R
    where
        T: Refines<()>,
        F: FnOnce(&mut V, &mut Child<Self, T, SG>)->R,
        V: Any+'static,
        SG: SubgraphBuilderT<(), T>,
    {
        ...
        let subscope = SG::new_from(addr, identifier, logging.clone(), summary_logging, name);
