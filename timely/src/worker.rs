...
pub trait AsWorker : Scheduler {
    ...
    /// channel.
    fn allocate<T: Data, A: Data+From<T>>(&mut self, identifier: usize, address: &[usize]) -> (Vec<Box<dyn Push<Message<T>, Message<A>>>>, Box<dyn Pull<Message<T>, Message<A>>>);
    ...
    fn pipeline<T: 'static, A: 'static+From<T>>(&mut self, identifier: usize, address: &[usize]) -> (ThreadPusher<Message<T>, Message<A>>, ThreadPuller<Message<T>, Message<A>>);
...
impl<A: Allocate> AsWorker for Worker<A> {
    ...
    fn allocate<D: Data, Al: Data+From<D>>(&mut self, identifier: usize, address: &[usize]) -> (Vec<Box<dyn Push<Message<D>, Message<Al>>>>, Box<dyn Pull<Message<D>, Message<Al>>>) {
    ...
    fn pipeline<T: 'static, Al: 'static+From<T>>(&mut self, identifier: usize, address: &[usize]) -> (ThreadPusher<Message<T>, Message<Al>>, ThreadPuller<Message<T>, Message<Al>>) {
        if address.is_empty() { panic!("Unacceptable address: Length zero"); }
        let mut paths = self.paths.borrow_mut();
        paths.insert(identifier, address.to_vec());
        self.temp_channel_ids.borrow_mut().push(identifier);
